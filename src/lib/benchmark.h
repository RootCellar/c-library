...
#define BENCHMARK_LOOPS(x, name, loops) do { struct timespec start_time = get_time();\
  ...
  printf("%s" name ": %'d over %'f seconds ( %'f per second ) " "%s\n", ANSI_COLOR_BRIGHT_CYAN, loops, seconds, ((float)loops/seconds), ANSI_COLOR_RESET); } while(0)
...
#define BENCHMARK_LOOPS_CODE(x, var, loops) do { struct timespec __start_time = get_time();\
  ...
  if(SHOW_ALL) printf("%s" #x ": %'d over %'f seconds ( %'f per second ) " "%s\n", ANSI_COLOR_BRIGHT_CYAN, loops, __seconds, __runs_per_second, ANSI_COLOR_RESET); var = __runs_per_second; } while(0)
